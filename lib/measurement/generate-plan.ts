...
export function buildPlanPrompt(
  ...
  const system = [
    ...
    '  "events": [{ "id": string, "name": string /*snake_case*/, "category": "page"|"engagement"|"ecommerce"|"form"|"conversion"|"custom", "triggerType": "formSubmit"|"click"|"linkClick"|"elementVisibility"|"historyChange"|"pageView"|"none" /*the ACTION kind, so GTM knows how to capture it: "formSubmit"=a form submission; "linkClick"=clicking a link incl. tel:/mailto:/whatsapp/outbound; "click"=clicking a button/element; "elementVisibility"=an element/offer becoming visible; "historyChange"=an SPA route/screen change; "pageView"=a page load; "none"=not a built-in-trigger action*/, "description": string, "trigger": string, "isKeyEvent": boolean, "requiresConsent": boolean, "parameters": [{ "name": string, "type": "string"|"number"|"boolean", "required": boolean, "description": string, "source": "dataLayer"|"gtm"|"page" /*WHERE the value comes from: "dataLayer"=dynamic app internal state GTM CANNOT read (monetary value, currency, item/product ids, dynamic names, counts); "page"=readable from the page/DOM/URL (visible text, a data-attribute, a query param); "gtm"=provided by GTM itself*/ }] }],',
