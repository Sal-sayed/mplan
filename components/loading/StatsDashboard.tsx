...
export default function StatsDashboard({ stats, mode }: Props) {
  ...
  return (
    <div className="bg-ds-card border border-ds-line rounded-xl shadow-sm h-full flex flex-col overflow-hidden">
      <div className="px-4 py-3 border-b border-ds-line">
        <div className="text-[10px] uppercase tracking-[0.15em] text-ds-secondary font-medium">
        ...
        {boxes.map((box, i) => {
          ...
          return (
            <motion.div
              ...
              className={`relative bg-ds-page border ${box.border} rounded-lg p-3 overflow-hidden`}
            >
              <div className={`pointer-events-none absolute -top-8 -right-8 h-20 w-20 rounded-full blur-2xl opacity-50 ${box.glow}`} />
              <Icon className={`w-4 h-4 ${box.iconText} mb-2`} strokeWidth={1.6} />
              <div className={`text-3xl font-semibold ${box.iconText} font-mono leading-none`}>
                <AnimatedNumber value={box.value} />
              </div>
              <div className="text-[9px] uppercase tracking-[0.1em] text-ds-muted mt-1 leading-tight">
                {box.label}
              </div>
            </motion.div>
          );
      ...
      <div className="px-4 py-2 border-t border-ds-line text-[10px] text-ds-muted flex items-center gap-1.5">
